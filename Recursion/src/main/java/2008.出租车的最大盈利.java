...
class Solution {
    HashMap<Integer, Long> cacheMap = new HashMap<>();
    HashMap<Integer, ArrayList<Integer[]>> ridesMap = new HashMap<>();
    ...
    public long dfs(int i) {
        ...
        if (ridesMap.containsKey(i)) {
            for (Integer[] arr : ridesMap.get(i)) {
                int start = arr[0];
                int cost = arr[1];
                // System.out.println(start);
                long res2 = dfs(start) + cost;
    ...
    public long maxTaxiEarnings(int n, int[][] rides) {
        ...
        for (int[] ride : rides) {
            ...
            Integer[] val = new Integer[] { start, end - start + tip };
            ArrayList<Integer[]> orDefault = ridesMap.getOrDefault(end, new ArrayList<>());
            orDefault.add(val);
            ridesMap.put(end, orDefault);
        }

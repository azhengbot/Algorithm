...
class StockPrice:
    def __init__(self):
        self.d: dict = dict()
        self.curr: int = 0
        self.curr_price: int = 0
        self.heap_max = []
        self.heap_min = []

    def update(self, timestamp: int, price: int) -> None:
        self.d[timestamp] = price
        if timestamp > self.curr or timestamp == self.curr:
            self.curr = timestamp
            self.curr_price = price

        heappush(self.heap_min, [price, timestamp])
        heappush(self.heap_max, [-price, timestamp])
    ...
    def maximum(self) -> int:
        ans = 0
        while self.heap_max:
            p, t = heappop(self.heap_max)
            if self.d[t] == -p:
                ans = -p
                heappush(self.heap_max, [-self.d[t], t])
                break
            heappush(self.heap_max, [-self.d[t], t])
    ...
    def minimum(self) -> int:
        while self.heap_min:
            p, t = heappop(self.heap_min)
            if self.d[t] == p:
                heappush(self.heap_min, [self.d[t], t])
                return p
            heappush(self.heap_min, [self.d[t], t])
        return 0

...
class Solution:
    def largestValsFromLabels(
        ...
        cnt = 0
        lv_tuple = sorted(list(zip(values, labels)), reverse=True)
        ans = 0

        for v, lab in lv_tuple:
...
numWanted = 3
useLimit = 2
res = s.largestValsFromLabels(
    values=values, labels=labels, numWanted=numWanted, useLimit=useLimit
)
print(res)

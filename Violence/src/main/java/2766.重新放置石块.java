...
class Solution {
    public List<Integer> relocateMarbles(int[] nums, int[] moveFrom, int[] moveTo) {
        ...
        for (int nu : nums) {
            int cnt = cntMap.getOrDefault(nu, 0);
            cnt++;
            cntMap.put(nu, cnt);
        }
        ...
        for (int i = 0; i < n; i++) {
            Integer a = cntMap.getOrDefault(moveFrom[i], 0);
            cntMap.put(moveFrom[i], 0);
            int b = cntMap.getOrDefault(moveTo[i], 0);
            b += a;
            cntMap.put(moveTo[i], b);
        }
        // System.out.println(cntMap);
        return cntMap.keySet()
                ...
                .filter(k -> {
                    ...
                    return cntMap.getOrDefault(k, 0) > 0;
                })
                .sorted()

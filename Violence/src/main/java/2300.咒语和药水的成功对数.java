...
class Solution {
    public int[] successfulPairs(int[] spells, int[] potions, long success) {
        ...
        Integer[][] spellsIndex = new Integer[n][2];

        for (int i = 0; i < n; i++) {
            spellsIndex[i] = new Integer[] { i, spells[i] };
        }

        Arrays.sort(spellsIndex, (o1, o2) -> Integer.compare(o1[1], o2[1]));
        ...
        for (int i = 0; i < n; i++) {
            Integer[] spell = spellsIndex[i];

            while (j >= 0) {
                Long a = spell[1].longValue();
                long b = potions[j];
